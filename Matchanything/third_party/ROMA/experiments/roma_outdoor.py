...
def train(args):
    ...
    model = get_model(
        pretrained_backbone=True, resolution=resolution, attenuate_cert=False
    ).to(device_id)
    # Num steps
    global_step = 0
    ...
    mega = MegadepthBuilder(
        data_root="data/megadepth", loftr_ignore=True, imc21_ignore=True
    )
...
def test_mega_8_scenes(model, name, resolution, sample_mode):
    ...
    mega_8_scenes_results = mega_8_scenes_benchmark.benchmark(
        model, model_name=name, scale_intrinsics=False
    )
...
if __name__ == "__main__":
    ...
    test_mega1500(
        model, experiment_name, resolution=test_resolution, sample_mode=sample_mode
    )
