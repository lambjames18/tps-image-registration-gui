...
if __name__ == '__main__':
    ...
    print(f"The Ransac Threshold is: {args.rigid_ransac_thr}")
    ...
    for npz_name in tqdm(npz_names):
        ...
        datasets.append(
            CommonDataset(data_root, npz_path, mode='test', min_overlap_score=-1, img_resize=args.imgresize, df=None, img_padding=False, depth_padding=True, testNpairs=None, fp16=False, load_origin_rgb=True, read_gray=True, normalize_img=False, resize_by_stretch=config.DATASET.RESIZE_BY_STRETCH, gt_matches_padding_n=100, dataset_name=args.dataset_name))
    ...
    save_h5(result_dict, (Path(args.output_path) / f'eval_{args.dataset_name}_{args.method}_{args.comment}_error.h5'))